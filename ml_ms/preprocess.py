...
def preprocess_data(raw_path, processed_path):
    ...
    categorical_cols = [c for c in ["special_event", "waste_category", "Season"] if c in df.columns]
    df = pd.get_dummies(df, columns=categorical_cols, drop_first=True)

    # --- Convert numeric columns ---
    numeric_cols = ["meals_served", "kitchen_staff", "temperature_C", "humidity_percent", "staff_experience", "food_waste_kg"]
    ...
    scale_cols = ["meals_served", "kitchen_staff", "temperature_C", "humidity_percent",
                  "staff_experience", "StaffPerMeal", "ExperiencePerMeal", "Temp_Humidity",
